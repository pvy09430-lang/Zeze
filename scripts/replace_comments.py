...
start_marker = "{(!detailedBotModal.comments || detailedBotModal.comments.length === 0) ? ("
end_marker = "{/* Bottom sticky Comment box */}"
...
new_content = """{isFetchingBotDetails ? (
                        <div className="space-y-6 animate-pulse">
                          {[1,2,3].map(i => (
                            <div key={i} className="flex gap-2">
                              <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-800 shrink-0"></div>
                              <div className="flex-1 space-y-2">
                                <div className="h-16 bg-slate-200 dark:bg-slate-800 rounded-2xl w-3/4"></div>
                            ...
                            <div className="flex-1 space-y-1">
                              ...
                              <div className="flex items-center gap-3 px-2 text-[10px] text-slate-500 font-medium">
                                ...
                                {(isAdminUnlocked || (c.userId && c.userId === userId) || (!c.userId && c.nickname === nickname)) && (
                                  <button onClick={() => handleDeleteComment(detailedBotModal.id, c.id)} className="hover:underline cursor-pointer">Xóa</button>
                                )}
                              </div>
                                      ...
                                      <div className="flex-1 space-y-1">
                                        ...
                                        <div className="flex items-center gap-3 px-2 text-[9px] text-slate-500 font-medium">
                                          ...
                                          {(isAdminUnlocked || (reply.userId && reply.userId === userId) || (!reply.userId && reply.nickname === nickname) || (c.userId && c.userId === userId) || (!c.userId && c.nickname === nickname)) && (
                                            <button onClick={() => handleDeleteCommentReply(detailedBotModal.id, c.id, reply.id)} className="hover:underline cursor-pointer">Xóa</button>
                                          )}
                                        </div>
                                      </div>
                                    </div>
                              ...
                              {activeCommentReplyId === c.id && (
                                <div className="mt-2 flex gap-1.5 items-center animate-fade-in pr-2">
                                  <input
                                    type="text"
                                    maxLength={120}
...
print("Successfully replaced comments block")
