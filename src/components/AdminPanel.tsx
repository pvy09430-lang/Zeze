...
function AdminPanel({
  ...
  // Visitor Log Filter & Search State
  const [logSearchQuery, setLogSearchQuery] = useState("");
  const [logFilterType, setLogFilterType] = useState<"all" | "clicks" | "comments" | "other">("all");
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {/* FULL-WIDTH VISITORS ACTIVITY LOG PANEL */}
      <div className="mt-8 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden">
        <div className="p-6 border-b border-slate-100 dark:border-slate-850 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/50 dark:bg-slate-950/20">
          <div className="flex items-center gap-2.5">
            <div className="p-2 bg-indigo-500/10 text-indigo-500 rounded-xl">
              <Activity className="w-5 h-5 animate-pulse" />
            </div>
            <div>
              <h3 className="font-display font-bold text-slate-800 dark:text-slate-100 text-lg flex items-center gap-1.5">
                Nhật Ký Hoạt Động Cổng Bot <Sparkles className="w-4 h-4 text-amber-500 fill-amber-500/20" />
              </h3>
              <p className="text-xs text-slate-450 mt-0.5">Dữ liệu lượt click bot, bình luận và hoạt động hệ thống được lưu trữ vĩnh viễn không xóa.</p>
            </div>
          </div>

          <div className="flex flex-wrap items-center gap-2">
            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Tổng số nhật ký:</span>
            <span className="px-2.5 py-0.5 bg-slate-150 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-extrabold text-[11px] rounded-full">
              {(state.visitorLogs || []).length} mục
            </span>
          </div>
        </div>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-slate-50 dark:bg-slate-950/40 p-3 rounded-xl border border-slate-100 dark:border-slate-850">
            {/* Search Input */}
            <div className="relative flex-1 max-w-md">
              <input
                type="text"
                placeholder="Tìm tên độc giả, tên bot hoặc nội dung hành động..."
                value={logSearchQuery}
                onChange={(e) => setLogSearchQuery(e.target.value)}
                className="w-full pl-3 pr-8 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-150 dark:focus:ring-indigo-950 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500"
              />
              {logSearchQuery && (
                <button
            ...
            <div className="flex flex-wrap gap-1.5">
              {[
                { id: "all", label: "Tất cả" },
                { id: "clicks", label: "Lượt nhấp bot 👆" },
                { id: "comments", label: "Bình luận 💬" },
                { id: "other", label: "Khác ⚙️" }
              ].map((pill) => (
                <button
                  key={pill.id}
                  onClick={() => setLogFilterType(pill.id as any)}
                  className={`px-3 py-1.5 text-[10.5px] font-bold rounded-lg transition-all cursor-pointer ${
                    logFilterType === pill.id
          ...
          {(() => {
            ...
            const filteredLogs = visitorLogs.filter((log) => {
              const matchesSearch = 
                log.nickname.toLowerCase().includes(logSearchQuery.toLowerCase()) ||
              ...
              const isClick = actionLower.includes("click") || actionLower.includes("nhấp") || actionLower.includes("xem");
              const isComment = actionLower.includes("bình luận") || actionLower.includes("phản hồi") || actionLower.includes("reply") || actionLower.includes("comment") || actionLower.includes("góp ý");
              
              if (logFilterType === "clicks") return isClick;
              if (logFilterType === "comments") return isComment;
              if (logFilterType === "other") return !isClick && !isComment;
            ...
            if (filteredLogs.length === 0) {
              return (
                <div className="text-center py-10 border border-dashed border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50/20 dark:bg-slate-900/10">
                  <Activity className="w-8 h-8 text-slate-350 dark:text-slate-600 mx-auto mb-2.5 animate-pulse" />
                  <p className="text-xs font-bold text-slate-500 dark:text-slate-400">Không tìm thấy hoạt động nào phù hợp</p>
                  <p className="text-[10px] text-slate-400 mt-1">Thử thay đổi từ khóa tìm kiếm hoặc chọn lọc thẻ khác.</p>
                </div>
              );
            }
            ...
            return (
              <div className="border border-slate-150 dark:border-slate-850 rounded-xl overflow-hidden">
                <div className="max-h-96 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-850 pr-1">
                  {filteredLogs.map((log) => {
                    const actionLower = log.action.toLowerCase();
                    const isClick = actionLower.includes("click") || actionLower.includes("nhấp") || actionLower.includes("xem");
                    const isComment = actionLower.includes("bình luận") || actionLower.includes("phản hồi") || actionLower.includes("reply") || actionLower.includes("comment") || actionLower.includes("góp ý");
                    
                    let actionBadgeStyle = "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-350";
                    if (isClick) {
                      actionBadgeStyle = "bg-green-50 dark:bg-green-950/30 text-green-600 dark:text-green-400 border border-green-500/10";
                    } else if (isComment) {
                      actionBadgeStyle = "bg-pink-50 dark:bg-pink-950/30 text-pink-600 dark:text-pink-400 border border-pink-500/10";
                    }

                    return (
                      <div key={log.id} className="p-3.5 hover:bg-slate-50/50 dark:hover:bg-slate-950/20 transition flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 text-xs">
                        <div className="flex items-start gap-2.5">
                          {/* Avatar icon */}
                          <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center font-bold text-white shadow-xs shrink-0 mt-0.5 uppercase">
                            {log.nickname.slice(0, 2)}
                          </div>
                          <div className="space-y-1">
                            <div className="flex flex-wrap items-center gap-1.5">
                              <span className="font-extrabold text-slate-800 dark:text-slate-150 hover:underline">
                                {log.nickname}
                              </span>
                              <span className={`text-[9.5px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${actionBadgeStyle}`}>
                                {isClick ? "Lượt click Bot" : isComment ? "Bình luận" : "Hoạt động"}
                              </span>
                            </div>
                            <p className="font-mono text-slate-600 dark:text-slate-350 text-[11px] leading-relaxed break-words pr-2">
                              {log.action}
                            </p>
                          </div>
                        </div>

                        {/* Date display without showing device */}
                        <div className="flex items-center gap-1.5 text-[10px] text-slate-400 font-medium sm:text-right shrink-0">
                          <Clock className="w-3.5 h-3.5 text-slate-300" />
                          <span>
                            {new Date(log.timestamp).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit", second: "2-digit" })} - {new Date(log.timestamp).toLocaleDateString("vi-VN", { day: "2-digit", month: "2-digit", year: "numeric" })}
                          </span>
                        </div>
                      </div>
                    );
